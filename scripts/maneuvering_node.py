...
class ThrustAllocationNewNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(f"Q: {self.maneuvering_allocator_.q_matrix}")
        self.get_logger().info(f"T: {self.maneuvering_allocator_.T}")
        self.get_logger().info(f"T_inv: {self.maneuvering_allocator_.T_pseudo_inv}")
    ...
    def tau_callback(self, msg: Wrench):
        ...
        xi_p, _, _, _ = self.thrust_allocator_.allocate(tau, dt)
        xi, u, alpha, tau_actual = self.maneuvering_allocator_.allocate(tau, xi_p, dt)
